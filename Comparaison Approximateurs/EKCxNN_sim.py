...
CKF_controller = ASTWC(time=simulation_time, Te=time_step, reference=None)
ckf = CKF(dynamic_model, sensor_model, initial_state_estimate, alpha=0.5)

# NN ASTWC initialization
NN_controller_EKF = EKF(dynamic_model, sensor_model, initial_state_estimate)
NN_inner_controller = ASTWC(time=simulation_time, Te=time_step, reference=None)
NN_controller = NN_based_STWC(controller=NN_inner_controller, time=simulation_time, Te=time_step, gamma=0.1)

# Initial state
CKF_controller.x1[0] = initial_state_estimate[0]
CKF_controller.x2[0] = initial_state_estimate[1]

NN_controller.controller.x1[0] = initial_state_estimate[0]
NN_controller.controller.x2[0] = initial_state_estimate[1]

# Simulation loop
for i, t in enumerate(tqdm(timeline)):  
    
    # Generate control input
    u_CKF = CKF_controller.compute_input(i) - ckf.perturbation_estimation[1]
    u_NN = NN_controller.compute_input(i)
    ...
    ckf_model_perturbation = dynamic_model.model_perturbation(ckf_x_true, t=t) 
    nn_model_perturbation = dynamic_model.model_perturbation(nn_x_true, t=t)
    
    ckf_perturbation = (dynamic_noise + ckf_model_perturbation) * time_step
    nn_perturbation  = (dynamic_noise + nn_model_perturbation) * time_step
    ...
    # CKF_controller prediction and correction steps
    ckf.prediction(u_CKF_with_noise)
    ckf.correction(y_ckf)
    ckf.estimate_perturbation()
    
    # NN_controller prediction and correction steps
    NN_controller_EKF.prediction(u_NN_with_noise)
    NN_controller_EKF.correction(y_nn)
    ...
    nn_estimated_perturbations.append(NN_controller.perturbation.clone())
    
    # Store the estimated state
    ckf_estimated_state = ckf.posterior_state_estimation.clone()
    ckf_estimated_states.append(ckf_estimated_state)
    CKF_controller.update_state(i, ckf_estimated_state)
    
    nn_estimated_state = NN_controller_EKF.posterior_state_estimation.clone()
    nn_estimated_states.append(nn_estimated_state)
    NN_controller.controller.update_state(i, nn_estimated_state)
...
ax1_5.plot(timeline, CKF_controller.s, label='Sliding Variable')
ax1_5.axhline(y=CKF_controller.epsilon, color='r', linestyle='--', label=f'epsilon = {CKF_controller.epsilon:.2f}')
ax1_5.axhline(y=-CKF_controller.epsilon, color='r', linestyle='--')
...
ax1_6.plot(timeline, CKF_controller.k[:-1], label='k')
...
ax2_5.plot(timeline, NN_controller.controller.s, label='Sliding Variable')
ax2_5.axhline(y=NN_controller.controller.epsilon, color='r', linestyle='--', label=f'epsilon = {NN_controller.controller.epsilon:.2f}')
ax2_5.axhline(y=-NN_controller.controller.epsilon, color='r', linestyle='--')
...
ax2_6.plot(timeline, NN_controller.controller.k[:-1], label='k')
